...
modisLstUpscaleTifDir = fullfile(dataDir, 'MYD11A1_3_UpscalingCn_TIF');
if ~exist(modisLstUpscaleTifDir, 'dir')
    mkdir(modisLstUpscaleTifDir)
end
modisLstUpscaleMatDir = fullfile(dataDir, 'MYD11A1_3_UpscalingCn_Matlab');
if ~exist(modisLstUpscaleMatDir, 'dir')
    mkdir(modisLstUpscaleMatDir)
end
...
%     yearStr = num2str(yearList(i));
    ...
    amsr2MaskMatExist = exist(amsr2MaskMatPath, 'file');
    ...
    modisMaskMatExist = exist(modisLstMaskMatPath, 'file');

    modisLstUpscaleMatName = sprintf('MYD11A1_UpscalingCn_%s_%s.mat', yearStr, dayNight);
    modisLstUpscaleMatPath = fullfile(modisLstUpscaleMatDir, modisLstUpscaleMatName);
    modisUpscaleMatExist = exist(modisLstUpscaleMatPath, 'file');
...
%         break
    ...
    % 创建存储升尺度, 以及掩膜后MODIS LST和AMSR2 BT数据的年路径.
    ...
    modisLstUpscaleTifYearDir = fullfile(modisLstUpscaleTifDir, modisLstYearFolder);
    if ~exist(modisLstUpscaleTifYearDir, 'dir')
        mkdir(modisLstUpscaleTifYearDir)
    end

    % 创建存储升尺度, 以及掩膜后年度MODIS LST和AMSR2 BT的矩阵.
    if  ~amsr2MaskMatExist || ~modisMaskMatExist || ~modisUpscaleMatExist
        [modisLstUpscaleYearArray, modisLstMaskYearArray] = ...
            deal(zeros(amsr2RowN, amsr2ColN, lstDateFilterN, 'single') * nan);
    ...
    for j = 1: lstDateFilterN
        % 升尺度后, 掩膜后的MODIS LST文件输出路径.
        modisLstFileName = split(modisLstPathFilterList{j}, '\');
        modisLstFileName = modisLstFileName{end};
        modisLstUpscaleFilePath = fullfile(modisLstUpscaleTifYearDir, modisLstFileName);
        modisLstMaskFilePath = fullfile(modisLstMaskTifYearDir, modisLstFileName);
        modisLstUpscaleFileExist = exist(modisLstUpscaleFilePath, 'file');
        modisLstMaskFileExist = exist(modisLstMaskFilePath, 'file');
        ...
        %   和Mat文件.
        if modisLstMaskFileExist && modisLstUpscaleFileExist && (amsr2MaskFileN == cpN)
            if modisUpscaleMatExist && modisMaskMatExist && amsr2MaskMatExist
                continue
            else
                modisLstUpscaleYearArray(:, :, j) = readgeoraster(modisLstUpscaleFilePath);
        ...
        % 输出升尺度后的MODIS LST数据.
        fprintf('输出%s %s的升尺度后MODIS LST数据.\n', lstDateFilterList{j}, dayNight)
        if ~modisLstUpscaleFileExist
            geotiffwrite(modisLstUpscaleFilePath, single(upscaledModisLstLayer), amsr2Ref, ...
        ...
        nanIndexLayer = isnan(upscaledModisLstLayer) | amsr2NanIndexLayer | ~cnMaskLayer;
        nanIndexArray = repmat(nanIndexLayer, 1, 1, channelN);
        modisLstMaskLayer = upscaledModisLstLayer;
        modisLstMaskLayer(nanIndexLayer) = nan;
        ...
        if ~modisLstMaskFileExist
            geotiffwrite(modisLstMaskFilePath, single(modisLstMaskLayer), amsr2Ref, ...
                TiffTags=struct('Compression','LZW'));
        end
        for k = 1: channelN
            if ~exist(amsr2MaskFilePathList{2*k-1}, 'file')
                geotiffwrite(amsr2MaskFilePathList{2*k-1}, amsr2HArray(:, :, k), amsr2Ref, ...
                    TiffTags=struct('Compression','LZW'));
            end
            if ~exist(amsr2MaskFilePathList{2*k}, 'file')
        ...
        if ~modisUpscaleMatExist || ~modisMaskMatExist || ~amsr2MaskMatExist
            modisLstUpscaleYearArray(:, :, j) = upscaledModisLstLayer;
            modisLstMaskYearArray(:, :, j) = modisLstMaskLayer;
            amsr2HMaskYearArray(:, :, startN: startN + channelN - 1) = amsr2HArray;
            amsr2VMaskYearArray(:, :, startN: startN + channelN - 1) = amsr2VArray;
            startN = startN + channelN;
        end
    ...
    if ~modisUpscaleMatExist || ~modisMaskMatExist || ~amsr2MaskMatExist
        ...
        save(modisLstUpscaleMatPath, 'amsr2Ref', 'modisLstUpscaleYearArray', 'lstDateFilterList');
        save(modisLstMaskMatPath, 'amsr2Ref', 'modisLstMaskYearArray', 'lstDateFilterList');
        save(amsr2MaskMatPath, 'amsr2Ref', 'amsr2*MaskYearArray', 'lstDateFilterList');
    end
end
