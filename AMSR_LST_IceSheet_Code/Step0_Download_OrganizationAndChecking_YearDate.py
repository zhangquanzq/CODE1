# coding=utf-8
# 组织和检查分年份, 按日期存储的数据, 涉及MODIS地表温度和反照率数据.
# 按数据类型, 年份, 日期分文件夹组织下载的MODIS数据, 并检查数据是否损坏, 缺失, 重复.
import arcpy
import numpy as np
import os
from os import path
...
urlTxtPath2 = path.join(urlTxtFolderPath, 'Rest_{0}_{1}{2}.txt'.format(modisType, yearNum, region))
