...
%     yearStr = num2str(yearList(i));
    ...
    load(amsrMatPath, 'amsr*YearArray', 'dateYearList');
    ...
    [validDateList, amsrDateIndex, maskDateIndex] = intersect(dateYearList, lstDateFilterList);
    validYearMonthList = datetime(validDateList , 'InputFormat', 'yyyyMMdd').Month;
    validDateN = length(validDateList);

    % 获取共有日期的Mask后的MODIS LST.
    modisLstMaskYearArray = modisLstMaskYearArray(:, :, maskDateIndex);
    ...
    for k = 1: channelN
        ...
        amsrHChannelArray = amsrHChannelArray(:, :, maskDateIndex);
        amsrVChannelArray = amsrVChannelArray(:, :, maskDateIndex);
    ...
    for j = 1: pureLcCodeN
        ...
        for k = 1 : channelN
            ...
            amsrHYearArray = amsrHYearArray(:, :, amsrDateIndex);
            amsrVYearArray = amsrVYearArray(:, :, amsrDateIndex);
